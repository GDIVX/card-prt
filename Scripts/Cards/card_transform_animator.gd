...
## Proper use:
	## When moving the card as a whole, animate_to the anchor and then call "reset". For example, the position of the card in the hand.
...
func reset(duration: float = 1) -> void:
...
## Remember to call reset() to recall the card to its resting state
